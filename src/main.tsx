...
function App() {
  ...
  const addSticker = (sticker: string) => setStickers([...stickers, sticker])
  return <main className="app-shell">
    ...
    <section className="paper-card"><div className="tape tape-left"/><div className="tape tape-right"/><div className="sticker-layer">{stickers.map((sticker, i) => <button className={`floating-sticker sticker-${i % 4}`} key={`${sticker}-${i}`} onClick={() => setStickers(stickers.filter((_, n) => n !== i))}>{sticker}</button>)}</div><div className="card-head"><span className="date-number">16</span><div><p>SEPTEMBER</p><small>星期三 · 农历八月初五</small></div><button className="pin"><MapPin size={16}/></button></div>
      ...
      <div className="diy-box"><div><p className="eyebrow">DIY STICKERS</p><strong>装饰这一页</strong></div><div className="sticker-palette"><button onClick={() => addSticker('✦')}>✦</button><button onClick={() => addSticker('♡')}>♡</button><button onClick={() => addSticker('☼')}>☼</button><button onClick={() => addSticker('✿')}>✿</button><button onClick={() => addSticker('☁')}>☁</button><button onClick={() => addSticker('★')}>★</button></div></div>
      <div className="long-note"><span>✎</span><div><b>小小的备注</b><p>把今天的心情、票根和灵感都贴在这里吧。每一页都可以长成自己的样子。</p></div></div>
